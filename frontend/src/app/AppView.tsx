...
interface AppViewProps {
  isMobile: () => boolean
  demoDropdownRef: RefObject<HTMLDivElement | null>
  showDemoDropdown: boolean
  setShowDemoDropdown: (value: boolean) => void
  loading: boolean
  analysis: Analysis | null
  computed: Computed | null
  availableYears: number[]
  selectedYears: number[]
  handleToggleYear: (year: number) => void
  handleResetYears: () => void
  summaryText: string
  filteredFilms: Film[]
  handleLoadDemoCSV: () => void
  handleLoadDemoReport: () => void
  handleUpload: (file: File) => Promise<void>
  lastUploadedFileName: string
  SHOW_MOCK_UI: boolean
  demoMockId: string
  setDemoMockId: (value: string) => void
  MOCK_OPTIONS: MockOption[]
  handleLoadDemo: () => void
  handleClearCache: () => Promise<void>
  cacheCleared: boolean
  lastReportAvailable: boolean
  handleOpenLastReport: () => Promise<void>
  showResumeModal: boolean
  resumeState: ResumeState | null
  handleResumeStartOver: () => void
  handleResumeContinue: () => void
  showMobileModal: boolean
  handleMobileCancel: () => void
  handleMobileContinue: () => Promise<void>
  progress: Progress | null
  handleCancelAnalysis: () => void
  retryMessage: string
  error: string
  setError: (value: string) => void
  posterSetIdsTop12: Set<number>
  simplifiedEmpty: boolean | undefined
}
