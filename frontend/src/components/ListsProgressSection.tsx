...
import { MOVIE_LISTS } from '../data/movieLists'
import type { Film } from '../types/film.types'

const R = 52
const CIRCUMFERENCE = 2 * Math.PI * R

interface ListsProgressSectionProps {
  films: Film[]
}

function ListsProgressSection({ films }: ListsProgressSectionProps) {
  const userTmdbIds = useMemo(
    () => new Set((films || []).map((f) => f.tmdb_id).filter((id): id is number => id !== null)),
    [films]
  )
