 ...
 */
export const COUNTRY_GLOBAL_FREQUENCY: Record<string, number> = {
 ...
 * 
 * @param countryName - Название страны на английском (как в TMDb production_countries)
 * @returns Глобальная частота страны в процентах (0-100), или null если страна не найдена
 */
export function getCountryGlobalFrequency(countryName: string | null | undefined): number | null {
  ...
  if (key === 'United States' || key === 'USA') {
    return COUNTRY_GLOBAL_FREQUENCY['United States of America'] ?? null
  }
  if (key === 'UK') {
    return COUNTRY_GLOBAL_FREQUENCY['United Kingdom'] ?? null
  }
  if (key === 'Czechia') {
    return COUNTRY_GLOBAL_FREQUENCY['Czech Republic'] ?? null
  }
  if (key === 'Russian Federation') {
    return COUNTRY_GLOBAL_FREQUENCY['Russia'] ?? null
  }
  return null
}
 ...
 * 
 * @returns Map: название страны → глобальная частота (%)
 */
export function createCountryGlobalFrequencyMap(): Map<string, number> {
